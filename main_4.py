...
def main():
    ...
    # Data loading code
    root_dir = args.data
    snr = args.data[:-4].split('/')[-1]
    # train_dir = root_dir + '-pre-raw-train.csv'
    # val_dir = root_dir + '-pre-raw-validate.csv'
    print(root_dir)
    x_train, x_valid, y_train, y_valid = feature_extraction(root_dir)
    ...
    dataset_train = DatasetTrain(x_train, y_train)
    ...
    validate dataset
    '''
    dataset_validate = DatasetTrain(x_valid, y_valid)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #val_class1_plot[epoch] = {val_class[0][0]}
        #val_class2_plot[epoch] = {val_class[1][0]}
        #val_class3_plot[epoch] = {val_class[2][0]}
        #val_class4_plot[epoch] = {val_class[3][0]}
        #val_class5_plot[epoch] = {val_class[4][0]}
        ...
        #data_save(directory + 'acc.txt', acc_plot)

        fold = directory + snr
        if not os.path.exists(fold):
            os.makedirs(fold)

        data_save(fold + '/acc.txt', acc_plot)

        #data_save(fold + '/val_class1.txt', val_class1_plot)
        #data_save(fold + '/val_class2.txt', val_class2_plot)
        #data_save(fold + '/val_class3.txt', val_class3_plot)
        #data_save(fold + '/val_class4.txt', val_class4_plot)
        #data_save(fold + '/val_class5.txt', val_class5_plot)
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            # val_class = report(output, target)
...
def report(output, target):
    ...
    with torch.no_grad():
        ...
        _, pred = output.topk(4, 1, True, True)
        pred = pred.t()
        correct = pred.eq(target.view(1, -1).expand_as(pred))
